        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                ...
                <div class="flex items-center gap-4 mb-6">
                    <img src="{{ asset($bungalow->image_url ?? 'https://placehold.co/100x75/E0E0E0/333333?text=Bungalow') }}" alt="{{ $bungalow->name }}" class="w-24 h-18 object-cover rounded-md shadow">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $bungalow->name }}</h3>
                        <p class="text-lg text-gray-700 dark:text-gray-300">Preço por noite: <span class="font-semibold text-green-600 dark:text-green-400">€{{ number_format($bungalow->price_per_night, 2, ',', '.') }}</span></p>
                    </div>
                </div>

                <form id="booking-form" action="{{ route('booking.confirm_and_pay') }}" method="POST" class="space-y-6">
                    @csrf
                    <input type="hidden" name="bungalow_id" value="{{ $bungalow->id }}">

                    <div>
                        <label for="start_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Data de Entrada</label>
                        <input type="text"
                               name="start_date"
                               id="start_date"
                               required
                    ...
                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Data de Saída</label>
                        <input type="text"
                               name="end_date"
                               id="end_date"
                               required
                    ...
                    <div class="flex items-center justify-between mt-6">
                        ...
                        <p class="text-xl font-bold text-gray-900 dark:text-gray-100">
                            Preço Total: <span id="total_price_display" class="text-green-600 dark:text-green-400">€0.00</span>
                        </p>
            ...
            document.getElementById('booking-form').addEventListener('submit', function(e) {
                ...
                .then(({ status, body }) => {
                    if (status >= 200 && status < 300) { // Success range
                        ...
                        let errorMessage = body.message || 'Ocorreu um erro inesperado. Por favor, tente novamente.';
                        if (body.errors) {
